...
( function( $ ) {
	...
	wp.customize( 'hdr_btn_bg_color', function( value ) {
		value.bind( function( bg_color ) {
			...
			jQuery( '.header_btn a' ).css( 'border-color', bg_color );
		} );
	} );
	 ...
	 * mobile_top_clr
	 */
	wp.customize( 'mobile_top_clr', function( value ) {
		value.bind( function( bg_color ) {
			var class_name      = 'customizer-topbar-color'; // Used as id in gfont link
			var css_class       = $( '.' + class_name );

			var head_append     = '<style class="' + class_name + '">@media (max-width: 992px){ .header .theme-mobile-nav {background-color: ' + bg_color + '; }}</style>';
	 ...
	 * Nav Bar Padding
	 */
	 range_live_media_load( 'xolo_menu_bar_padding', '#header-section .navigation, #header-section .theme-mobile-nav', [ 'padding-top', 'padding-bottom' ], 'px' );
